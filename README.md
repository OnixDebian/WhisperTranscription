...
- Choose a Whisper model (tiny → large-v3, plus turbo)
- Cap **CPU threads** and set a **RAM ceiling** (`RLIMIT_AS`)
...
- The RAM cap uses `RLIMIT_AS`. The kernel may refuse to lower the limit
  once the model is loaded — set it before pressing **Start**. A failure to
  apply the limit is shown in the status bar but does not stop the run.
